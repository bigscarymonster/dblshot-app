/*
------------------------------------------------------------------------------ 
This code was generated by Amplication. 
 
Changes to this file will be lost if the code is regenerated. 

There are other ways to to customize your code, see this doc to learn more
https://docs.amplication.com/docs/how-to/custom-code

------------------------------------------------------------------------------
  */
import { ArgsType, Field } from "@nestjs/graphql";
import { ApiProperty } from "@nestjs/swagger";
import { CoffeeBrandWhereInput } from "./CoffeeBrandWhereInput";
import { Type } from "class-transformer";
import { CoffeeBrandOrderByInput } from "./CoffeeBrandOrderByInput";

@ArgsType()
class CoffeeBrandFindManyArgs {
  @ApiProperty({
    required: false,
    type: () => CoffeeBrandWhereInput,
  })
  @Field(() => CoffeeBrandWhereInput, { nullable: true })
  @Type(() => CoffeeBrandWhereInput)
  where?: CoffeeBrandWhereInput;

  @ApiProperty({
    required: false,
    type: [CoffeeBrandOrderByInput],
  })
  @Field(() => [CoffeeBrandOrderByInput], { nullable: true })
  @Type(() => CoffeeBrandOrderByInput)
  orderBy?: Array<CoffeeBrandOrderByInput>;

  @ApiProperty({
    required: false,
    type: Number,
  })
  @Field(() => Number, { nullable: true })
  @Type(() => Number)
  skip?: number;

  @ApiProperty({
    required: false,
    type: Number,
  })
  @Field(() => Number, { nullable: true })
  @Type(() => Number)
  take?: number;
}

export { CoffeeBrandFindManyArgs };
